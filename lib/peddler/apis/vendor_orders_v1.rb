...
module Peddler
  module APIs
    ...
    class VendorOrdersV1 < API
      ...
      # will return purchase orders which have one or more items cancelled by Amazon with updated item quantity as zero.
      ...
      # If the item receive status is not included in the filter, purchase orders for all receive statuses are included.
      # @param ordering_vendor_code [String] Filters purchase orders based on the specified ordering vendor code. This
      ...
      # providing ship to location id here. This value should be same as 'shipToParty.partyId' in the purchase order. If
      # not included in filter, this will return purchase orders for all the buyer's warehouses used for vendor group
      # purchase orders.
      # @param rate_limit [Float] Requests per second
      # @return [Peddler::Response] The API response
