...
module Peddler
  module APIs
    ...
    class OrdersV0 < API
      ...
      # 8601](https://developer-docs.amazon.com/sp-api/docs/iso-8601) format. **Note**: `CreatedBefore` is optional when
      # `CreatedAfter` is set. If specified, `CreatedBefore` must be equal to or after the `CreatedAfter` date and at
      # least two minutes before current time.
      ...
      # when `LastUpdatedAfter` is set. But if specified, `LastUpdatedBefore` must be equal to or after the
      ...
      # payment has not been authorized, and the release date of the item is in the future.) - `Pending` (The order has
      # been placed but payment has not been authorized.) - `Unshipped` (Payment has been authorized and the order is
      # ready for shipment, but no items in the order have been shipped.) - `PartiallyShipped` (One or more, but not
      # all, items in the order have been shipped.) - `Shipped` (All items in the order have been shipped.) -
      ...
      # were paid with the specified payment methods. **Possible values**: `COD` (cash on delivery), `CVS` (convenience
      # store), `Other` (Any payment method other than COD or CVS).
      ...
      # cannot be delivered.) - `ReturningToSeller` (The package was not delivered and is being returned to the seller.)
      # - `ReturnedToSeller` (The package was not delivered and was returned to the seller.) - `Lost` (The package is
      # lost.) - `OutForDelivery` (The package is out for delivery.) - `Damaged` (The package was damaged by the
      # carrier.)
      ...
      # `Errored` (The last submitted electronic invoice was rejected for this order.) - `Accepted` (The last submitted
      # electronic invoice was submitted and accepted.)
      ...
      # (or at) a specified time. The date must be in [ISO 8601](https://developer-docs.amazon.com/sp-api/docs/iso-8601)
      # format.
